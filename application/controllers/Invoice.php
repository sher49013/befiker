...
//		$data['order_payment_info'] = $this->InvoiceModel->get_all_order_payment_by_id($order_id);
	...
	{
		$this->InvoiceModel->order_status($order_id);
		$this->session->set_flashdata("flsh_msg","<font class='success'>Order Status change Successfully</font>");
		redirect('manage-order');

	}

	public function payment_order($payment_id)
	{
		$this->InvoiceModel->payment_status($payment_id);
		$this->session->set_flashdata("flsh_msg","<font class='success'>Payment Status change Successfully</font>");
		redirect('manage-order');
